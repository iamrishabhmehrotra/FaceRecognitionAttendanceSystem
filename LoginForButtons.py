...
class Login_Atten:
    ...
    def reset_pass(self):
        if self.combo_sequrity_Q.get()=="Select":
            ...
            if row==None:
                ...
                conn.commit()
                conn.close()
                messagebox.showinfo("Successfully","Your Password changed successfully",parent=self.root)
    ...
    def forgate_password_window(self):
        if self.txtuser.get()=="":
            ...
            if row==None:
                messagebox.showerror("Error","Please enter valid user name!",parent=self.root2)
            else:
                conn.close()
                self.root2=Toplevel()
                self.root2.title("Forget Password")
