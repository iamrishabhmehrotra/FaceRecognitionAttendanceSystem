...
class Student:
    def __init__(self, root):
        ...
        current_course_frame = LabelFrame(Left_frame, bd=2, bg="white", relief=RIDGE, text="Current course information",
                                          font=("times new roman", 12, "bold"))
        current_course_frame.place(x=5, y=50, width=605, height=115)
        ...
        studentName_label = Label(class_Student_frame, text="Student Name:", font=("times new roman", 12, "bold"),
                                  bg="white")
        studentName_label.grid(row=0, column=2, padx=3, pady=5, sticky=W)
        ...
        # class_div_label = Label(class_Student_frame, text="Student Devision:", font=("times new roman", 12, "bold"),
        #                         bg="white")
        ...
        #                             font=("times new roman", 12, "bold"))
        # class_div_entry.grid(row=1, column=1, padx=3, pady=5, sticky=W)
        class_div_label = Label(class_Student_frame, text="Student Devision:", font=("times new roman", 12, "bold"),
                                bg="white")
        class_div_label.grid(row=1, column=0, padx=3, pady=5, sticky=W)
        ...
        div_combo["values"] = ("Select Devision", "A", "B", "C", "D", "E", "F", "G")
        div_combo.current(0)
        ...
        phone_label.grid(row=3, column=2, padx=3, pady=5, sticky=W)

        phone_entry = ttk.Entry(class_Student_frame, textvariable=self.var_phone, width=20,
                                font=("times new roman", 12, "bold"))
        phone_entry.grid(row=3, column=3, padx=3, pady=5, sticky=W)
        ...
        btn_frame1.place(x=3, y=260, width=590, height=37)

        # Take photo sample button
        take_photo_btn = Button(btn_frame1, text="Take Photo Sample", command=self.genrate_dataset, width=25,
                                font=("times new roman", 13, "bold"),
                                bg="blue", fg="white")
        take_photo_btn.grid(row=0, column=0, padx=15)

        update_photo_btn = Button(btn_frame1, text="Update Photo Sample",command=self.genrate_dataset, width=25,
                                  font=("times new roman", 13, "bold"),
                                  bg="blue", fg="white")
        ...
        self.student_table = ttk.Treeview(table_frame, column=(
            'dep', 'cursor', 'year', 'sem', 'id', 'name', 'div', 'roll', 'gender', 'dob', 'email', 'phone', 'address',
        ...
        self.student_table.heading("cursor", text="Cursor")
        ...
        # 'dep','cursor','year','sem','id','name','div','roll','gender','dob','email','gender','phone','address','teacher','photo'
        self.student_table.column("dep", width=100)  # do it for all names
        self.student_table.column("cursor", width=100)
    ...
    def add_data(self):
        ...
        self.Phone = re.match(self.pattern_phone, self.var_phone.get())
        self.pattern_email = r"^[A-Za-z]+[-_$.A-Za-z]*[0-9]*@[A-Za-z]*\.[A-Za-z]+$"
        self.Email = re.match(self.pattern_email, self.var_email.get())
        self.pattern_dob = r"^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}$"
        self.DOBVLI = re.match(self.pattern_dob,self.var_dob.get())


        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "" or self.var_gender.get() == "Select Gender" or self.var_div.get() == "Select Devision" or self.var_roll.get() == "":
            messagebox.showerror("Error", "All Fields are required!", parent=self.root)

        elif self.var_course.get() == "Select Course" or self.var_year.get() == "Select Year" or self.var_semester.get() == "Select Semester":
            messagebox.showerror("Error", "All Fields are required!", parent=self.root)

        elif not self.Phone or self.var_phone.get() == "":
        ...
        elif not self.DOBVLI or self.var_dob.get() == "":
            messagebox.showerror("Error","Please enter date of birth in formate DD/MM/YYYY or D/M/YYYY")
        elif self.validate_roll_and_search()==False:
            messagebox.showerror("Error","These Roll No is already present in selcted class try another Roll No!",parent=self.root)
    ...
    def update_data(self):
        ...
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "" or self.var_gender.get() == "Select Gender" or self.var_div.get() == "Select Devision" or self.var_roll.get() == "":
            ...
            messagebox.showerror("Error", "Please enter date of birth in formate DD/MM/YYYY or D/M/YYYY",parent=self.root)
    ...
    def delete_data(self):
        if self.var_std_id.get == "":
            ...
            try:
                delete = messagebox.askyesno("Student Delete Page", "Do you want to delete these student details!",
                                             parent=self.root)
                if delete > 0:
                    conn = mysql.connector.connect(host="localhost", username="root", password="pass",
                                                   database="face_recognizer")
    ...
    def fetch_student_data(self):
        ...
        try:
            cursor.execute("SELECT Roll, Semester, Dep, Division, Year FROM student")
            student_data = cursor.fetchall()
            return student_data
        finally:
            conn.close()

    def validate_roll_number(self, roll_no, semester, department, division, years, student_data):
        for roll, sem, dept, div, year in student_data:
            if roll_no == roll and semester == sem and department == dept and division == div and years==year:
                return False  # Roll number is already assigned for the same semester, department, and division
        return True  # Roll number is valid

    def validate_roll_and_search(self):
        ...
        if self.validate_roll_number(roll_no, semester, department, division, years, student_data):
            return True
        else:
            return False
